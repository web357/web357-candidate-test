...
// Add the custom CSS file only for your component
$doc->addStyleSheet(JURI::base() . 'components/com_web357test/media/css/component.css');
	...
	<table class="table">
		...
		<tr>
			...
			<td>
				...
				{
					echo Text::_('COM_WEB357TEST_RECIPE_SERVING_SIZE_OPTION_' . preg_replace('/[^A-Za-z0-9\_-]/', '', strtoupper(str_replace(' ', '_', $this->item->serving_size))));
				}
				?>
			</td>
		</tr>
		...
		<tr>
			...
			<td title="<?php echo $this->item->difficulty; ?>" >
				<?php
				if (!empty($this->item->difficulty)) {
					// Convert difficulty level to a human-readable format
					$difficultyText = htmlspecialchars($this->item->difficulty, ENT_QUOTES, 'UTF-8');

					// Define Font Awesome icons based on difficulty
					$icons = '';
					switch (strtolower($this->item->difficulty)) {
						case '1-easy':
							$icons = '<i class="fas fa-star" aria-hidden="true"></i>';
							break;
						case '2-medium':
							$icons = '<i class="fas fa-star" aria-hidden="true"></i> <i class="fas fa-star" aria-hidden="true"></i>';
							break;
						case '3-hard':
							$icons = '<i class="fas fa-star" aria-hidden="true"></i> <i class="fas fa-star" aria-hidden="true"></i> <i class="fas fa-star" aria-hidden="true"></i>';
							break;
						default:
							$icons = htmlspecialchars($this->item->difficulty, ENT_QUOTES, 'UTF-8'); // Fallback for unknown values
					}
				?>
					<!-- Hidden text for screen readers -->
					<span class="visually-hidden"><?php echo $difficultyText; ?></span>

					<!-- Icons with accessibility support -->
					<span role="img" aria-label="<?php echo $difficultyText; ?>">
						<?php echo $icons; ?>
					</span>
				<?php } ?>
			</td>
